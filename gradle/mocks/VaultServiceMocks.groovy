...
vaultToken = new File('.credentials').text.tokenize('\n').find { it.startsWith('VAULT_TOKEN=') }?.with { it.tokenize('=')[1].trim() }
TokenCredential creds = [getToken: {-> vaultToken }] as TokenCredential
